...
if (require.main === module) {
  scrapeBetPawaKenya().then(events => {
    // Log stats only to stderr
    console.error(`BETPAWA KENYA SCRAPER STATS: Found ${events.length} events`);
    
    if (events.length > 0) {
      // Log extra debug info to stderr only
      console.error(`BETPAWA KENYA STATS: Total upcoming events scraped: ${events.length}`);
      
      // IMPORTANT: Output ONLY the JSON data to stdout, nothing else
